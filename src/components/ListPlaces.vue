		...
		<div class="container">
			...
			<div class="col4">
				...
				<div class="numresultsblock">
					<span id="numresults">{{ numFilteredItems }}</span> lieux
				</div>
				<div class="entitiesresults">
					<div v-for="item in filteredItems">
						<a :href="'http://localhost:5173/offline/edit/ca_places_' + item.id"><img src='/public/edit.png' style="height:22px;padding-right:8px;" align="top" /></a> 
						<!-- {{ item.idno }} -->
...
export default {
	...
	mounted() {
		...
		$.each(localStorage, function (key, value) {
			...
			if (key.startsWith("ca_places_") && (data['idno'] != null)) {
				$('#content ul').append($("<a href='/offline/places/edit/" + key + "'><li>" + data['idno'] + "</li></a>"));
				that.data.push(data);
				i++;
			}
		});
